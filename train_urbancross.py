...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.test_step > 0 and (epoch + 1) % args.test_step == 0:
            rsum_, all_scores_ = engine.validate(args, test_loader, model)
...
def generate_random_samples(args):
    """
    Generate random samples from the training data and split them into train and validation sets.
    ...
    # Create directory for data info
    ...
    # Copy train and validation set to data directory
    ...
    # Visualize and calculate data set split
...
def generate_stratified_random_samples(args):
    """
    Generate stratified random samples from the training data and split them into train and validation sets.
    ...
    # Calculate number of samples for each class in the training set
    ...
    # Split data into train and validation sets based on class distribution
...
def update_options_savepath(args, k):
    """
    Update the save path in the arguments based on the current fold number.
    ...
    Returns:
        argparse.Namespace: Updated arguments with the save path modified.
    """
