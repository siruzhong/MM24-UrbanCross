...
def setup_seed(seed):
    ...
    torch.manual_seed(seed)  # Set the seed for PyTorch on CPU
    torch.cuda.manual_seed(seed)  # Set the seed for PyTorch on GPU
    torch.cuda.manual_seed_all(seed)    # Set the seed for all GPUs
    os.environ['PYTHONHASHSEED'] = str(seed)  # Prevent hash randomization
    torch.backends.cudnn.deterministic = True  # Ensure consistent convolution algorithms
